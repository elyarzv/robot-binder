...
static bool shouldStartBag = true;
...
int main(int argc, char** argv) {
    ...
    ros::param::get("~data_directory", data_dir);
    ros::param::get("~file_prefix", file_prefix);
    // nh.getParam("~data_directory", data_dir);
    // nh.getParam("~file_prefix", file_prefix);
    ROS_INFO_STREAM("data dir: " << data_dir << " fileprefix: " << file_prefix);
    ...
    if(!boost::filesystem::exists(out_dir)) {
       ROS_WARN("Output directory doesnt exists, creating it %s",data_dir.c_str());
